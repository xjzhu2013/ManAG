function [X, out] = GrassmannAG1(fun, X0, opts, varargin)
% Exponential-based accelerated gradient method on Grassmann manifold
% Input: fun - objective 
%        X0 - initial point
...
for k = 1:mxitr
    ...
    if trig
        ...
        Xi = invParaGr(Grad, U, V, c, s, Z);
    end
    
    % BB stepsize for alpha
    if k > 1
        %S = invRetrSt(X, Xp);
        S = Xp - X;
